...
13.85 degree H-APNR target tabulated in the cited study. The `5 x 6` ribbon is
wider and shorter so the complete edit remains legible without a long sequence
of repetitive steps. After the ninth commit, the camera moves from above the
ribbon to below it so the completed three-dimensional twist can be inspected.
...
3. Close the panel, left-drag a new box from the **third ridge through the
   ...
   coordinates.
4. Continue advancing the real box-selection boundary by one 12-atom ridge.
   After 9 backend commits, the final ridge is rotated by exactly 13.85 degrees.
5. The completed structure is then viewed from above through to below; this is
   a camera orbit only and does not alter the committed coordinates.
...
example uses a short, wide `5 x 6` repeat and one half-cell ridge per step
(12 atoms at this ribbon width), rather than rotating both ridges together. Green and
...
The top-view example uses a coherent `6 x 6` primitive Cu2O(111) mesh on
`7 x 7 Cu(111)`.
...
ASE's conventional Cu2O(111) surface cell spans two primitive surface
translations, so the constructor's `3 x 3` repeat is the `6 x 6` primitive
oxide mesh. One lowest-layer oxide O is explicitly registered over a
top-layer substrate Cu atom; the other eight interface O atoms then sample the
full coincidence phase. The scene is unrelaxed and applies about 1.22%
in-plane compression to the oxide. It demonstrates interface-aware pair
filtering, not a substitute for DFT relaxation.

The `3 x 3` coincidence overgrowth and `-1.22%` mismatch follow the experimental
[Cu2O(111)/Cu(111) growth study](https://doi.org/10.1016/0022-0248(78)90299-3).
The supported-film DFT model reports that relaxation bonds unsaturated
interfacial O to substrate Cu at top, bridge, and hollow sites
([DOI 10.1021/acs.jpcc.0c04453](https://doi.org/10.1021/acs.jpcc.0c04453)).
The pristine oxide surface construction is cross-checked against
[DOI 10.1039/C8CP06023A](https://doi.org/10.1039/C8CP06023A).
