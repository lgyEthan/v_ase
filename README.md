...
[![Version](https://img.shields.io/badge/version-0.2.34a1%2Bsymmetry-d2a84a.svg)](CHANGELOG.md)
...
> the independent version `0.2.34a1+symmetry`, and is not published to PyPI.
> Install it from the `symmetry` branch as shown below.

The version format is `MAIN_BASEaSYMMETRY_ITERATION+symmetry`. Therefore,
`0.2.34a1+symmetry` means this build is synchronized with the v_ase `0.2.34`
main viewer state and starts the first symmetry alpha iteration on that base.
...
The arrows use **Phonon equilibrium** as their displacement reference: each
arrow starts at the unperturbed supercell site and ends at the atom's current
harmonic position. They therefore reverse direction across half a period
instead of measuring against an arbitrary trajectory frame.
