...
* - `vase_export_video`
  - Export video from the shared GUI to a unique local artifact. container is mov, avi or gif and requires a loaded trajectory. startFrame/endFrame are inclusive zero-based source indices (default entire trajectory); loop controls GIF…
* - `vase_export_poscar`
