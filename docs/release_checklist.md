...
1. Advance `MAIN_BASEaSYMMETRY_ITERATION+symmetry` in `pyproject.toml`,
   `v_ase/_version.py`, the application shell, cache-busted JavaScript imports,
   and `CHANGELOG.md`. `MAIN_BASE` must identify the exact synchronized main
   release; the alpha number advances only for symmetry-branch work.
...
7. When rendering or constraint visuals change, run
   ...
   manifest and inspect every Analysis-panel PNG and animation phase.
...
9. Run the complete test suite, build wheel and source distribution, and run
   ...
   present.
10. Keep the tested release local unless the user explicitly requests a push
    to the remote `symmetry` branch. Never push this build to `main`, create a
    production release tag, or upload it to PyPI.
11. If a branch documentation preview is configured, verify it resolves the
    `symmetry` commit and does not replace the production `stable` alias.
12. Install the locally built wheel in a clean environment and verify
