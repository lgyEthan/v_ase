...
problem in v_ase 0.2.35a1+symmetry. Unless stated otherwise, installation and
repair commands run from the checked-out `symmetry` branch root.
...
```bash
python -m pip install --upgrade -e ".[symmetry,phonon]"
python -m v_ase.cli --version
```
...
```bash
python -m pip install --upgrade --force-reinstall -e ".[symmetry,phonon]"
```

For this build, Python 3.10-3.12 resolve the NumPy 1.x/matscipy 1.1.x family;
