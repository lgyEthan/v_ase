...
Additional local software-rendered smoke passed against the public 0.4.7 wheel
using the CI-verified Vulkan loader and device scale factor 1. All existing
checks passed; destination readiness took 2,616 ms. This is Apple-silicon
SwiftShader evidence, not a substitute for the separate Intel CI gate.

Final desktop CI run [36088034748](https://github.com/lgyEthan/v_ase/actions/runs/36088034748)
passed all three targets at `2a8b5a1c2099b2b6439209088cba5428538f1dd1`.
Development/packaged detach readiness was 1,027/1,099 ms on Mac arm64,
38,922/42,014 ms on native Intel software rendering, and 3,423/3,422 ms
on Windows. Every result retains all sixteen behavioral flags, ten commands,
69 layout routes and a nonblank exact-size render. Windows installer tests
confirmed optional scientific-file registration, preserved existing defaults,
and clean removal on uninstall. Final Intel and Windows packaged screenshots
were visually inspected. Both Mac packages contain all 103 scientific-package
files byte-identical to the published wheel. Windows has 82 byte-identical
files and 21 canonical Skill text files differing only in checkout CRLF line
endings; all code and scientific assets are identical.

The Apple-silicon candidate passed signed and post-stapling packaged smoke,
including all sixteen behavioral flags. Detach readiness was 528/586 ms.
Apple accepted app submission `5a5a0c23-9069-46b2-8084-a974ee34e7bf` and
DMG submission `468a76cb-c2f0-4805-aa9d-276fb4bd1dc5`. Both tickets and
Gatekeeper assessments passed. Every one of the 273 bundled Mach-O files
passed Developer ID, secure-timestamp and hardened-runtime verification;
the bundle contained no generated bytecode caches.

Some local signing-Mac attempts were interrupted or failed. A diagnostic
replay directly observed keyboard events that the smoke script had not sent;
those runs are excluded from release evidence, and raw key values were removed
from the local diagnostic log. With external input paused, the unmodified
Intel signed bundle passed both pre-notarization and post-stapling smoke.
The signed application was not modified for the diagnostic. A final installation-guide
review also removed a contradictory old paragraph about `.vase` mode prompts;
README and the later section already described direct saved-mode opening.

Intel's additional signing-Mac tests used Rosetta, SwiftShader and device scale
factor 1; native Intel execution is independently covered by CI. Signed/final
detach readiness was 33,701/30,929 ms, with every behavioral, layout and render
assertion passing. Apple accepted app submission
`fa45bb52-0153-4e26-961c-e99c96f05411` and DMG submission
`08f288bd-1399-4333-bb19-47d995ef6788`. Both tickets and Gatekeeper assessments,
all 273 native binaries, timestamps, hardened runtime, bundle integrity and
absence of bytecode caches passed. Final workspace and render images were
visually inspected.

## Public delivery verification

All ten desktop distribution/evidence assets and `desktop-SHA256SUMS.txt`
were uploaded to the 0.4.7 GitHub release. Every asset was downloaded again
from its public URL and matched both GitHub's SHA-256 digest and the local
post-stapling manifest. The downloaded Mac ZIPs passed every native-component
and notarization check again. Both downloaded DMGs passed signature, ticket
and Gatekeeper checks, mounted read-only, and contained a valid 0.4.7 app and
the Applications installation shortcut. Windows EXE/ZIP bytes remain the
successful CI candidates; Windows publisher signing is not claimed.

Read the Docs release build `34750748` succeeded at the immutable Python tag
`3ae466565c5cdb8da7f0a7d1d15ff5798b925427`. The corrected installation guide's
latest build `34751336` succeeded at `bc5bad74169cf72b31605f117a2880c831fbce61`;
its served content was checked for saved-mode opening and all three installer
links. Versioned search and PDF/ePub assets were checked earlier in this
release. Final strict Sphinx HTML and linkcheck builds passed, including the
now-public installer URLs. The final release notes link to the corrected
latest installation guide. Subsequent validation-record-only commits do not
change either the immutable Python release or the recorded desktop candidate.
...
- [x] All three native/packaged desktop CI targets at the recorded final commit.
- [x] Both Mac architectures Developer ID signed, notarized and stapled;
      packaged smoke, all native-component verification and Gatekeeper checks.
- [x] Public installers, ZIPs, source, signing evidence and final checksums;
      re-download verification. Windows unsigned status remains explicit.
- [x] Published Read the Docs version/latest and final strict linkcheck.
