...
def make_ai_pyridinic_graphene_scene() -> tuple[Atoms, Atoms, dict[str, object]]:
    """Return a deterministic graphene-to-N3-vacancy editing example."""
    ...
    final.set_chemical_symbols(symbols)
    set_atom_labels(final, labels)
    final.info.update({
...
def build_scene(name: str) -> tuple[Atoms, SceneInfo]:
    ...
    if name == "ai-edit":
        ...
        info = SceneInfo(
            name=name,
            description="AI-generated pyridinic N3 vacancy in a 6 x 6 ASE graphene sheet.",
            static_file="ai_pyridinic_n3_graphene.cif",
            selected_indices=tuple(idx["neighbors_after"]),
            notes=(
                "The source lattice is generated by ase.build.graphene; no external coordinates are copied.",
                "The AI workflow deletes the central C and converts its three nearest neighbors to N.",
            ),
        )
        return atoms, info
    if name == "materials":
    ...
    for name in scene_names:
        ...
        if name == "phosphorene":
            ...
            extra_assets = [
                ...
                ("ai_pyridinic_n3_graphene.traj", static_atoms),
            ]
        else:
