...
Native macOS and Windows windows around the published v_ase 0.4.5 GUI. Scientific
...
its SHA-256, installs published `v_ase-gui[mcp,rhino]==0.4.5` and locked binary
...
ditto -c -k --sequesterRsrc --keepParent "$VASE_APP" "$VASE_OUTPUT/v_ase-0.4.5-mac-$VASE_ARCH.zip"
...
codesign --force --timestamp --sign "$VASE_IDENTITY" "$VASE_OUTPUT/v_ase-0.4.5-mac-$VASE_ARCH.dmg"
xcrun notarytool submit "$VASE_OUTPUT/v_ase-0.4.5-mac-$VASE_ARCH.dmg" --keychain-profile vase-notary --wait --output-format json
```
...
```sh
xcrun stapler staple "$VASE_OUTPUT/v_ase-0.4.5-mac-$VASE_ARCH.dmg"
xcrun stapler validate "$VASE_OUTPUT/v_ase-0.4.5-mac-$VASE_ARCH.dmg"
codesign --verify --deep --strict "$VASE_APP"
python scripts/verify_macos.py "$VASE_APP" --team-id YOUR_TEAM_ID --notarized
spctl --assess --type open --context context:primary-signature --verbose=2 "$VASE_OUTPUT/v_ase-0.4.5-mac-$VASE_ARCH.dmg"
```
...
5. Use authenticated `gh release upload v0.4.5 --repo lgyEthan/v_ase --clobber`
