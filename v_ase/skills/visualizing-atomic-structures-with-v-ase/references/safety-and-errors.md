...
| Stop or wait for placement relaxation before adding more content | A repeated scatter was requested while the shared optimizer still owns staged coordinates | Run `stop-relaxation` or wait for `addAtoms.is_relaxing:false`, then append the next batch without finishing the session |
...
| `Could not determine the file format` | ASE could not infer a reader from the filename/content | Choose the exact Reader or use a recognized name/extension; numbered VASP names such as `XDATCAR_2` are already auto-detected identically in CLI and Open File |
