...
The tested branch version is `0.0.120a5+symmetry`. The `0.0.120` prefix records
the main viewer state from which this repository was forked, while `a5` is the
independent symmetry iteration. It is not a PyPI release; never substitute a
`v_ase-gui==...` registry install for this checkout.
