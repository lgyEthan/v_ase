...
from .ai_schema import (
    AI_CONTROL_SCHEMA, AI_OPERATION_PARAMETERS, AI_EXPORT_PARAMETERS,
    AI_DESCRIBE_PROFILES, AI_RENDER_PARAMETERS, ai_schema_payload,
    _AI_COMMAND_METHODS, _AI_COMMAND_DEFAULT_TIMEOUT_SECONDS,
)
from .project import (
    PROJECT_MIME,
    SETTINGS_SCHEMA,
...
def normalize_ai_command(payload: Dict[str, Any]) -> tuple[str, Any, float]:
    ...
    if method in {"schema", "query", "describe", "capabilities", "apply", "render", "export", "activate"} \
...
async def dispatch_ai_browser_command(
    ...
    *,
    ...
    try:
        connection = ws_manager.command_connection(session_id=session_id, session_prefix=session_prefix)
    except ValueError as exc:
    ...
    try:
        await connection.send_json({
            "type": "ai_command", "protocol": AI_PROTOCOL,
            "command_id": command_id, "method": method, "params": params,
...
async def append_structure_path(session_id: str, payload: Dict[str, Any]):
    ...
    display_name = _validated_uploaded_filename(source_path.name)
    input_format = payload.get("input_format") or None
    if payload.get("expected_source_kind") == "volumetric" and not resolve_volumetric_format(source_path, input_format):
        raise HTTPException(status_code=400, detail="The requested path is not a supported volumetric format. No frames were appended.")
    index = str(payload.get("index") or ":")
    volumetric_precision = str(payload.get("volumetric_precision") or "float32")
    try:
        return await _append_session_from_file(
...
@app.get("/api/settings/read-path/{session_id}")
async def read_visual_settings_path(session_id: str, path: str):
    from .project import normalize_visual_settings
    session = get_session(session_id)
    source = _resolve_launch_path(session, path, require_file=True)
    if source.stat().st_size > 64 * 1024 * 1024:
        raise HTTPException(status_code=413, detail="Visual settings exceed the 64 MiB input limit.")
    try:
        settings = normalize_visual_settings(json.loads(source.read_text(encoding="utf-8")))
    except (ValueError, OSError) as exc:
        raise HTTPException(status_code=400, detail=f"Could not read visual settings: {exc}") from exc
...
if FASTAPI_AVAILABLE:
    ...
    @app.post("/api/export/video-frames/{session_id}/start")
    async def start_video_frames(session_id: str, payload: Dict[str, Any]):
        from .video_frames import VideoFrameEncoder
        session = get_session(session_id)
        with session.mode_transition_lock:
            if session.video_exports:
                raise HTTPException(status_code=409, detail="A video export is already active for this document.")
            if session.is_relaxing:
                raise HTTPException(status_code=409, detail="Stop optimization before exporting a trajectory.")
            try:
            ...
            session.video_exports[export_id] = encoder
        return {"export_id": export_id, "width": encoder.width, "height": encoder.height,
                "fps": encoder.fps, "frames": encoder.frames}

    def video_frame_encoder(session_id: str, export_id: str):
        session = get_session(session_id)
        encoder = session.video_exports.get(export_id)
        if encoder is None:
            raise HTTPException(status_code=404, detail="This video export is unknown or already closed.")
        return session, encoder

    @app.post("/api/export/video-frames/{session_id}/{export_id}/frame/{index}")
    async def append_video_frame(session_id: str, export_id: str, index: int, request: Request):
        _, encoder = video_frame_encoder(session_id, export_id)
        data = bytearray()
        async for chunk in request.stream():
            data.extend(chunk)
            if len(data) > MAX_UPLOADED_IMAGE_BYTES:
                raise HTTPException(status_code=413, detail="Video frame exceeds the 512 MiB input limit.")
        try:
            return await asyncio.to_thread(encoder.append, index, bytes(data))
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        except VideoExportError as exc:
            raise HTTPException(status_code=500, detail=str(exc)) from exc

    @app.post("/api/export/video-frames/{session_id}/{export_id}/finish")
    async def finish_video_frames(session_id: str, export_id: str):
        session, encoder = video_frame_encoder(session_id, export_id)
        try:
            path, filename, mime = await asyncio.to_thread(encoder.finish)
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        except VideoExportError as exc:
            raise HTTPException(status_code=500, detail=str(exc)) from exc
            ...
            if encoder.closed:
                session.video_exports.pop(export_id, None)
        return FileResponse(path, filename=filename, media_type=mime,
                            headers={"X-V-Ase-Frames": str(encoder.count),
                                     "X-V-Ase-Width": str(encoder.width), "X-V-Ase-Height": str(encoder.height)},
                            background=BackgroundTask(_remove_temporary_file, path))

    @app.post("/api/export/video-frames/{session_id}/{export_id}/abort")
    async def abort_video_frames(session_id: str, export_id: str):
        session = get_session(session_id)
        encoder = session.video_exports.pop(export_id, None)
        if encoder is not None:
            await asyncio.to_thread(encoder.abort)
        return {"aborted": encoder is not None}
